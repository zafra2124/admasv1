/*
  # Fix notification type casting and system setup

  1. Database Functions
    - Fix calculate_ticket_results_for_month function with proper column qualification
    - Fix trigger_calculate_results function with proper enum casting

  2. System Configuration
    - Insert default system settings
    - Create sample winning number for current month
    - Add welcome notifications for existing users
    - Create utility function for sample data

  3. Security
    - All functions use proper error handling
    - Enum types are properly cast
...
-- Fix the trigger function to properly cast enum types
CREATE OR REPLACE FUNCTION trigger_calculate_results()
RETURNS trigger AS $$
BEGIN
  -- Calculate results for the new winning number
  PERFORM calculate_ticket_results_for_month(NEW.month);
  
  -- Notify users about new results (with proper enum casting)
  ...
  SELECT 
    DISTINCT t.user_id,
    'new_draw'::notification_type,  -- Fix: Cast to proper enum type
...
-- Create system notification templates (with proper enum casting)
INSERT INTO notifications (
  user_id,
  type,
  title,
...
SELECT 
  p.user_id,
  'system'::notification_type,  -- Fix: Cast to proper enum type
  'Welcome to Lottery Checker!',
  'Thank you for joining our lottery tracking system. Start by adding your first ticket!',
  '{"welcome": true, "version": "1.0.0"}'::jsonb
FROM profiles p
WHERE NOT EXISTS (
  SELECT 1 FROM notifications n 
  WHERE n.user_id = p.user_id 
  AND n.type = 'system'::notification_type
